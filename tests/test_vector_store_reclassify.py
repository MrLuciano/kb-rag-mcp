    ...
    mock_qdrant_client.scroll.return_value = (
        [
            PointStruct(
                id="chunk-1",
                vector=[0.1] * 384,
                payload={"source_file": "docs/test.pdf"},
            ),
            PointStruct(
                id="chunk-2",
                vector=[0.2] * 384,
...
@pytest.mark.asyncio
async def test_update_chunk_metadata_filter_built_correctly(
    mock_qdrant_client,
):
    ...
    source_file_condition = [
        c for c in scroll_filter.must if c.key == "source_file"
    ][0]
    assert source_file_condition.match.value == "docs/test.pdf"

    # Verify chunk_index condition
    chunk_index_condition = [
        c for c in scroll_filter.must if c.key == "chunk_index"
    ][0]
