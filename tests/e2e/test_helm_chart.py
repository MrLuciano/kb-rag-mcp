...
def test_monitoring_enabled_by_default():
    ...
    prometheus_docs = [
        d
        for d in docs
        if d.get("metadata", {}).get("name", "").endswith("-prometheus")
    ]
    grafana_docs = [
        d
        for d in docs
        if d.get("metadata", {}).get("name", "").endswith("-grafana")
    ]
...
def test_monitoring_can_be_disabled():
    ...
    prometheus_docs = [
        d
        for d in docs
        if "prometheus" in d.get("metadata", {}).get("name", "")
    ]
    grafana_docs = [
        d for d in docs if "grafana" in d.get("metadata", {}).get("name", "")
    ]
    assert (
        len(prometheus_docs) == 0
    ), "Prometheus resources found when monitoring.enabled=false"
    assert (
        len(grafana_docs) == 0
    ), "Grafana resources found when monitoring.enabled=false"
...
def test_prometheus_statefulset_has_pvc():
    ...
    prometheus_sts = [
        ...
        and "prometheus" in d.get("metadata", {}).get("name", "")
    ]
...
def test_grafana_has_datasource_configmap():
    ...
    datasource_cm = [
        ...
        and "grafana-datasources" in d.get("metadata", {}).get("name", "")
    ]
...
def test_grafana_has_dashboard_configmap():
    ...
    dashboard_cm = [
        ...
        for d in docs
        if d.get("kind") == "ConfigMap"
        and "grafana-dashboards" in d.get("metadata", {}).get("name", "")
        and "provisioning" not in d.get("metadata", {}).get("name", "")
    ]
...
def test_helm_lint_passes():
    ...
    result = subprocess.run(
        ["helm", "lint", str(CHART_PATH)], capture_output=True, text=True
    )
