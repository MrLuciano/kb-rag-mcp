...
def test_prometheus_scrape_target():
    ...
    assert (
        "kb-rag-mcp:8080" in targets
    ), f"Prometheus not scraping kb-rag-mcp, found: {targets}"
...
def test_grafana_dashboard_provisioning():
    ...
    with open(db_config) as f:
        config = yaml.safe_load(f)
    assert (
        config["providers"][0]["options"]["path"] == "/etc/grafana/dashboards"
    )


@pytest.mark.skipif(
    subprocess.run(["which", "docker-compose"], capture_output=True).returncode
    != 0,
