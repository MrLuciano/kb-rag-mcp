...
async def test_embedder_single():
    with patch("qa.embedder.get_embedding", new=AsyncMock(return_value=FAKE_VEC)):
...
async def test_embedder_batch():
    with patch("qa.embedder.get_embeddings_batch", new=AsyncMock(return_value=[FAKE_VEC, FAKE_VEC])):
        from qa.embedder import Embedder
        embedder = Embedder()
