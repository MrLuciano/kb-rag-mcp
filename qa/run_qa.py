...
def parse_args():
    parser = argparse.ArgumentParser(description="Run OTCS QA pipeline end-to-end.")
    parser.add_argument("--otcs-path", type=str, help="Path to OTCS docs root (for ingest)")
...
async def run_ingest_stage(otcs_path, workers):
    from ingest.ingest import run_ingest
    os.environ["QDRANT_COLLECTION"] = "qa_otcs"
    print(f"Ingesting documents from {otcs_path} ...")
    await run_ingest(Path(otcs_path), "OTCS", workers, clean=True, force=True)
...
async def run_pipeline(args):
    try:
        if not args.eval:
            await run_ingest_stage(args.otcs_path, args.workers)
