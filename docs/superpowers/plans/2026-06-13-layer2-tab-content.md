# Layer 2: Admin Tab Content

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement all 6 admin tab content panels — Admin Config page, Monitor Lights bar, Ingestion (manual + schedule + monitor), RAGAS (golden set editor + evaluation), Browser Cleanup (delete + re-ingest), Grafana embed, and Profile page.
...
**Tech Stack:** FastAPI, Jinja2, HTMX, Alpine.js, Bootstrap 5, `asyncio`, existing `JobManager`, existing `GoldenDataset`.

---

### Task 1: Admin Config page (tab_admin.html)

**Files:**
- Modify: `kb_server/ui/routes_admin.py`
- Modify: `kb_server/ui/templates/admin/tab_admin.html`

- [ ] **Step 1: Write the failing test — config tab loads and displays**

`tests/test_admin_ui.py` (append):
```python
@pytest.mark.asyncio
...
- [ ] **Step 2: Implement config tab template**
...
- [ ] **Step 3: Implement inline config editing in routes_admin.py**

Add to `kb_server/ui/routes_admin.py` — a helper to render config as an HTML table:
...
Create `kb_server/ui/templates/admin/_config_table.html`:
```html
{% if configs %}
...
```bash
git add kb_server/ui/routes_admin.py kb_server/ui/templates/admin/tab_admin.html kb_server/ui/templates/admin/_config_table.html
git commit -m "feat(layer2): add admin config page with inline editing"
```

---

### Task 2: Monitor Lights bar

**Files:**
- Modify: `kb_server/ui/routes_admin.py`
- Create: `kb_server/ui/templates/admin/_monitor_lights.html`
...
- [ ] **Step 2: Add monitor-lights endpoint**

Add to `kb_server/ui/routes_admin.py`:
```python
...
- [ ] **Step 3: Create monitor lights template**
...
```bash
git add kb_server/ui/routes_admin.py kb_server/ui/templates/admin/_monitor_lights.html
git commit -m "feat(layer2): add monitor lights bar with 7 component health indicators"
```

---

### Task 3: Ingestion tab (manual + schedule + monitor)

**Files:**
- Create: `kb_server/ui/templates/admin/_ingestion_manual.html`
- Create: `kb_server/ui/templates/admin/_ingestion_schedule.html`
- Create: `kb_server/ui/templates/admin/_ingestion_monitor.html`
- Create: `ingest/scheduler.py`
- Modify: `kb_server/ui/routes_admin.py`
...
- [ ] **Step 2: Add ingestion tab endpoints to routes_admin.py**
...
- [ ] **Step 3: Create manual ingestion template**
...
- [ ] **Step 5: Create monitor template (with progress polling)**
...
- [ ] **Step 6: Create IngestionScheduler**
...
Add schedule API endpoints to `routes_admin.py`:
```python
...
```bash
git add ingest/scheduler.py kb_server/ui/routes_admin.py kb_server/ui/templates/admin/_ingestion_*.html
git commit -m "feat(layer2): add ingestion tab with manual, schedule, and monitor sub-tabs"
```

---

### Task 4: RAGAS tab — golden set editor and evaluation

**Files:**
- Modify: `kb_server/evaluation/dataset.py`
- Create: `kb_server/ui/templates/admin/_ragas_editor.html`
- Create: `kb_server/ui/templates/admin/_ragas_results.html`
- Modify: `kb_server/ui/routes_admin.py`

- [ ] **Step 1: Add CRUD methods to GoldenDataset**

Append to `kb_server/evaluation/dataset.py`:
```python
class GoldenDataset:
    # ... existing methods ...
...
- [ ] **Step 3: Add RAGAS endpoints to routes_admin.py**
...
- [ ] **Step 4: Create RAGAS editor template**
...
- [ ] **Step 5: Create RAGAS results template**
...
```bash
git add kb_server/evaluation/dataset.py kb_server/ui/routes_admin.py kb_server/ui/templates/admin/_ragas_*.html
git commit -m "feat(layer2): add RAGAS tab with golden set editor and evaluation runner"
```

---

### Task 5: Browser Cleanup (delete + re-ingest)

**Files:**
- Modify: `kb_server/ui/templates/browse.html`
- Modify: `kb_server/ui/routes_admin.py`
- Create: `kb_server/ui/templates/admin/_browse_cleanup.html`

- [ ] **Step 1: Add cleanup API endpoints**
...
Add checkbox column and action bar. Replace the existing `<table>` section in browse.html. Add before the table:
...
```bash
git add kb_server/ui/routes_admin.py kb_server/ui/templates/browse.html
git commit -m "feat(layer2): add browser cleanup with delete and re-ingest"
```

---

### Task 6: Grafana embed in Monitoring tab

**Files:**
- Modify: `kb_server/ui/templates/admin/tab_monitoring.html`
- Modify: `kb_server/ui/routes_admin.py`

- [ ] **Step 1: Create Grafana helper functions**
...
- [ ] **Step 2: Update monitoring tab template**

Replace `tab_monitoring.html`:
```html
<h3>Monitoring</h3>
...
- [ ] **Step 3: Make helper functions available to templates**

In `kb_server/ui/app.py`, register the helper functions as Jinja2 globals:
```python
...
```bash
git add kb_server/ui/routes_admin.py kb_server/ui/templates/admin/tab_monitoring.html kb_server/ui/app.py
git commit -m "feat(layer2): add Grafana dashboard embed with time range selector"
```

---

### Task 7: Profile tab — personal API keys, export, erasure

**Files:**
- Create: `kb_server/ui/templates/admin/_profile_content.html`
- Modify: `kb_server/ui/routes_admin.py`

- [ ] **Step 1: Create profile content template**
...
- [ ] **Step 2: Add profile content endpoint**
...
- [ ] **Step 3: Add users/me endpoints to auth router**

In `kb_server/auth/router.py`, add the export and erasure-request endpoints for `users/me`:
...
async def export_my_data(current_user: User = Depends(get_current_user)):
    ...
    request = current_user  # We need the Request object
...
```bash
git add kb_server/ui/templates/admin/_profile_content.html kb_server/ui/routes_admin.py kb_server/auth/router.py
git commit -m "feat(layer2): add profile tab with API key management, data export, and erasure"
```
