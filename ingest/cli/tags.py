...
@click.option(
    "--product", type=str, default=None, help="Filter by product"
)
@click.option(
    "--type", "doc_type", type=str, default=None, help="Filter by doc type"
)
@click.option(
    "--vendor", type=str, default=None, help="Filter by vendor"
)
def list_tags(
...
async def _list_tags_impl(
    ...
    query = f"SELECT path, tags, product, doc_type FROM files {where_clause}"
    ...
    if not tag_counts:
        console.print("[yellow]No tags found on matching documents.[/yellow]")
        return
...
def update_tags(
    ...
    asyncio.run(
        _update_tags_impl(
            add_tags, remove_tags, replace_tags, filter_expr,
            collection, dry_run, yes,
        )
    )
...
async def _update_tags_impl(
    ...
    # Parse tags
    add_list = _validate_tags(
        [t.strip() for t in (add_tags_str or "").split(",") if t.strip()]
    ) if add_tags_str else []
    remove_list = _validate_tags(
        [t.strip() for t in (remove_tags_str or "").split(",") if t.strip()]
    ) if remove_tags_str else []
    replace_list = _validate_tags(
        [t.strip() for t in (replace_tags_str or "").split(",") if t.strip()]
    ) if replace_tags_str else []

    if not add_list and not remove_list and not replace_list:
        console.print("[red]Error: No tags specified. Use --add, --remove, or --replace.[/red]")
...
def remove_documents(
    ...
    asyncio.run(
        _remove_documents_impl(filter_expr, collection, dry_run, yes)
    )
...
async def _delete_tag_impl(
    ...
    for f in matched:
        try:
            ...
            error_count += 1
            log.error(
                f"Failed to remove tag from {f['path']}: {e}"
            )
