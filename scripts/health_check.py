...
async def check_embedding():
    ...
    from embed_client import health_check, BACKEND, MODEL, LMS_BASE_URL, LMS_HOST, LMS_PORT
    ...
    if result["status"] == "ok":
        ...
        if BACKEND == "openai-compat":
            ...
            print(f"    curl http://{LMS_HOST}:{LMS_PORT}/api/v0/embeddings \\")
            print(f"      -H 'Content-Type: application/json' \\")
            print(f"      -d '{{\"model\":\"{MODEL}\",\"input\":\"test\"}}'")
        print()
        print("  Variáveis no .env:")
        print("    EMBED_BACKEND=openai-compat  ← recomendado para LM Studio remoto")
        print(f"    LMS_BASE_URL=http://<ip>:1234  ← SEM /v1 ou /api/v0 no final")
        print(f"    EMBED_MODEL=<nome exato do modelo no LM Studio>")
        return False


async def check_qdrant():
    print("\n[2/3] Testando conexão com Qdrant...")
    from vector_store import VectorStore, QDRANT_HOST, QDRANT_PORT, QDRANT_PATH
    if QDRANT_PATH:
...
async def check_search(store):
    ...
    try:
        ...
        if results:
            ...
            print(f"  Top resultado: {results[0]['source_file']} (score: {results[0]['score']:.2f})")
        else:
...
async def main():
    ...
    ok_search = await check_search(store) if (ok_embed and ok_qdrant) else False
